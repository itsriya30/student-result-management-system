...
class Login:
    def __init__(self, home):
        ...
        Frame_login=Frame(self.home,bg="#F5E6CC")
        Frame_login.place(x=500,y=100,height=550,width=500)

        # Login title
        title=Label(Frame_login,text="User Login",font=("Times New Roman",25,"bold"),fg="dark red",bg="#F5E6CC").place(x=150,y=20)
        
        #Username and password lable
        lbl_user=Label(Frame_login,text="User Name",font=("Times New Roman",15,"bold"),fg="black",bg="#F5E6CC").place(x=45,y=140)
        self.txt_user=Entry(Frame_login,font=("times new roman",15),bg="white")
        self.txt_user.place(x=45,y=170,width=400,height=35)


        lbl_pass=Label(Frame_login,text="Password",font=("Times New Roman",15,"bold"),fg="black",bg="#F5E6CC").place(x=45,y=240)
        self.txt_pass=Entry(Frame_login,font=("times new roman",15),bg="white")
        self.txt_pass.place(x=45,y=270,width=400,height=35)

        forget=Button(Frame_login,text="Forget Password?",cursor="hand2",command=self.forget_window,bg="white",fg="dark red",font=("times new roman",15,"bold")).place(x=320,y=320)
        
        #Buttons
        signup=Button(Frame_login,text="Sign Up",cursor="hand2",command=self.register_window,bg="#A67B5B",fg="white",font=("times new roman",12,"bold")).place(x=220,y=480,height=40)

        login=Button(Frame_login,command=self.login_function,text="Login",cursor="hand2",bg="#8B4513",fg="white",font=("times new roman",20,"bold")).place(x=210,y=370,width=100,height=50)

        lbl_create=Label(Frame_login,text="Create an account?",font=("Times New Roman",15,"bold"),fg="black",bg="#F5E6CC").place(x=170,y=450)

        #Checkbuttons
        UserType=Label(Frame_login,text="User : ", font=("times new roman", 15, "bold"), bg="#F5E6CC",fg="black").place(x=45,y=85)
        ...
        if self.txt_user.get()=="":
            ...
            try:
                ...
                if row==None:
                    ...
                    self.home2.configure(bg="#F5E6CC")
                    self.home2.focus_force()
                    self.home2.grab_set()
                #Title for Forget Interface
                    title=Label(self.home2,text="Forget Password",font=("Times New Roman",25),fg="dark red",bg="#F5E6CC").place(x=0,relwidth=1)
